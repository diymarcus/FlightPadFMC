...
public class CDUView extends View {
    ...
    // ─── 7 hardcoded keepers — iFly737MAX's values reused verbatim ─────────────
    // FMC737 NG3 skin shares the same bezel geometry as iFly's skin (Marcus
    // derived iFly's by erasing NG3-only labels). Top-bar UI + EXEC LED slot +
    // 4 annunciator label strips land in pixel-identical positions.
    ...
    // CDU character-grid screen rect (fixed by 737 skin geometry, same as iFly).
    ...
    // 737 has CALL (left top, white) where 777 has DSPY. Each label is
    // rendered as stacked vertical text. Indices preserved for MainActivity's
    // setAnnunciatorState() contract.
    ...
    private static final float[][] ANN_RECTS = {
        ...
        ANNUN_LEFT_TOP_RECT,     // CALL — left top
        ANNUN_LEFT_BOTTOM_RECT,  // FAIL — left bottom
    };
    // PMDG sim render colours (verified 2026-05-07): MSG white, OFST amber,
    // CALL white, FAIL red. Same render across NG3 + 777 — Marcus confirmed
    // FAIL is red on both (older guess that NG3 was amber was wrong).
    private static final int[] ANN_COLOR_ON = {
        Color.rgb(255, 255, 255),  // MSG  — white
    ...
    // DEBUG: force all 4 bezel annunciators ON for visual verification.
    ...
    // 69 bezel-button hints — same table as tools/scan_buttons_preview.py
    // APPS["FMC737"]["hints"]. Pixel-shared positions with iFly737MAX (same
    // skin geometry — Marcus derived iFly's skin from this NG3 bezel) but
    // FMC737-specific labels (CLB/CRZ/DES + MENU/DEP at row 2 + N1LIMIT/FIX
    // at cluster 1, vs iFly's DEP ARR/ATC/VNAV + FIX/FMC COMM + MENU/N1 LIMIT).
    // MAN + PREV MENU dropped same as iFly (Marcus's call 2026-05-07 — buttons
    // stay printed on the bezel art but become non-functional).
    private static final Hint[] BUTTON_HINTS = {
        ...
        // Function row 1: INIT, RTE, CLB, CRZ, DES (NG3 layout)
        new Hint("INIT", 0.1635f, 0.5430f, 0.055f, 0.025f, RR),
        new Hint("RTE",  0.2922f, 0.5411f, 0.055f, 0.025f, RR),
        new Hint("CLB",  0.4149f, 0.5393f, 0.055f, 0.025f, RR),
        new Hint("CRZ",  0.5405f, 0.5393f, 0.055f, 0.025f, RR),
        new Hint("DES",  0.6642f, 0.5418f, 0.055f, 0.025f, RR),
        // Function row 2: MENU, LEGS, DEP, HOLD, PROG, EXEC
        new Hint("MENU", 0.1675f, 0.5969f, 0.055f, 0.025f, RR),
        new Hint("LEGS", 0.2882f, 0.5981f, 0.055f, 0.025f, RR),
        new Hint("DEP",  0.4149f, 0.5969f, 0.055f, 0.025f, RR),
        new Hint("HOLD", 0.5405f, 0.5969f, 0.055f, 0.025f, RR),
        new Hint("PROG", 0.6632f, 0.5981f, 0.055f, 0.025f, RR),
        new Hint("EXEC", 0.8295f, 0.6018f, 0.055f, 0.025f, RR),
        // Cluster row 1: N1LIMIT, FIX
        new Hint("N1LIMIT", 0.1635f, 0.6588f, 0.055f, 0.025f, RR),
        new Hint("FIX",     0.2882f, 0.6563f, 0.055f, 0.025f, RR),
    ...
    private void init(Context context) {
        logD("init() started - PMDG 737 NG3 version");
        ...
        try {
            ...
            if (skin != null) {
                logD("737 skin loaded: " + (skin.getByteCount() / 1024) + "KB ("
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        if (skin != null) {
            ...
            if (scrRatio < imgRatio) {
                ...
                imgOffsetX = 0; imgOffsetY = (h - drawH) / 2f;   // centred
            } else {
     ...
     *  Same v6 algorithm as iFly737MAX + FMC777_OWN: search rect, grey&lt;60
     *  mask, 2x2 binary opening, biggest connected component bbox.
     *  CLR-bottom-clamped-to-/ special case preserved. */
